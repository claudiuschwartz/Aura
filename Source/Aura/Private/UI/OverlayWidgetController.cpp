...
void UOverlayWidgetController::BindCallbacksToDependencies()
{
	...
	Cast<UAuraAbilitySystemComponent>(AbilitySystemComponent)->EffectAssetTags.AddLambda(
		...
		// Bind with a LAMBDA FUNCTION .AddLambda( [] () {} ) // the sequence of [] () {} is the signature for a lambda.
		{
			for (const FGameplayTag& Tag : AssetTags)
			{
				...
				{
				// const FString Msg = FString :: Printf(TEXT("GE Tag: %s"), *Tag.ToString()); - DEBUG 
                // GEngine->AddOnScreenDebugMessage(-1, 8.f, FColor:: Green, Msg); - DEBUG
